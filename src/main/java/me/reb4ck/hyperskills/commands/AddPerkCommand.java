...
public class AddPerkCommand extends HyperCommand {

    public AddPerkCommand() {
        super(Collections.singletonList("addperk"), "Add more player's perk", "hyperskills.addperk", false, "/Skills addperk [player] [perk] [quantity]");
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        if (args.length == 4) {
            try {
                ...
                if (player != null) {
                    HyperSkills.getInstance().getApi().addPerk(player.getUniqueId(), perk, quantity);
                    if(sender instanceof Player)
                        sender.sendMessage(StringUtils.color(HyperSkills.getInstance().getMessages().getMessage("addedPerk")
